...
var import_youtube_dl_exec = __toESM(require("youtube-dl-exec"), 1);
...
var requireAuth = async (req, res, next) => {
  ...
  try {
    ...
    const isPhoneAdmin = adminPhones.some(
      (p) => cleanPhone.endsWith(p) || cleanPhone === p || decodedToken.uid && decodedToken.uid.includes(p)
    );
    if (isPhoneAdmin) {
      assignedRole = "admin";
    }
...
var import_youtube_scraper = __toESM(require("@vreden/youtube_scraper"), 1);
...
var import_node_media_server = __toESM(require("node-media-server"), 1);
import_dotenv.default.config();
var firebaseConfig2 = {};
try {
  const configPath = import_path2.default.join(process.cwd(), "firebase-applet-config.json");
  if (import_fs2.default.existsSync(configPath)) {
    firebaseConfig2 = JSON.parse(import_fs2.default.readFileSync(configPath, "utf-8"));
  }
} catch (e) {
...
var db = (0, import_firestore.getFirestore)(void 0, firebaseConfig2.firestoreDatabaseId || "(default)");
...
function getStripe() {
  if (!stripeClient) {
    ...
    stripeClient = new import_stripe.default(key, { apiVersion: "2025-01-27.acacia" });
  }
  return stripeClient;
}
...
function getRazorpay() {
  if (!razorpayClient) {
    ...
    razorpayClient = new import_razorpay.default({ key_id, key_secret });
  }
...
app.use((0, import_helmet.default)({
  frameguard: false,
  // Allow iframe rendering in AI Studio preview (crucial for development preview tool inside iframe)
  ...
  contentSecurityPolicy: {
    directives: {
      ...
      // Allow rendering in AI Studio iframe
    }
  },
...
var CSRF_SECRET = process.env.CSRF_SECRET || import_crypto.default.randomBytes(32).toString("hex");
function generateCsrfToken() {
  const token = import_crypto.default.randomBytes(24).toString("hex");
  const signature = import_crypto.default.createHmac("sha256", CSRF_SECRET).update(token).digest("hex");
  return `${token}.${signature}`;
}
function verifyCsrfToken(tokenWithSignature) {
  try {
    if (!tokenWithSignature || typeof tokenWithSignature !== "string") return false;
    const parts = tokenWithSignature.split(".");
    if (parts.length !== 2) return false;
    const [token, signature] = parts;
    const expectedSignature = import_crypto.default.createHmac("sha256", CSRF_SECRET).update(token).digest("hex");
    const sigBuffer = Buffer.from(signature);
    const expectedBuffer = Buffer.from(expectedSignature);
    if (sigBuffer.length !== expectedBuffer.length) return false;
    return import_crypto.default.timingSafeEqual(sigBuffer, expectedBuffer);
  } catch (err) {
...
app.post("/api/payments/verify", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (provider === "stripe") {
      finalAmount = parseFloat((rawAmount / 83).toFixed(2));
      finalCurrency = "USD";
...
app.post("/api/admin/verify-role", async (req, res) => {
  try {
    ...
    const isPhoneAdmin = ADMIN_PHONES.some((p) => cleanPhone.endsWith(p) || cleanPhone === p || uid && String(uid).includes(p));
...
app.get("/api/yt-stream", async (req, res) => {
  ...
  try {
    try {
      const vredenData = await import_youtube_scraper.default.ytmp4(url);
    ...
    if (data.is_live) {
      const format2 = data.formats.find((f) => f.protocol === "m3u8" || f.protocol === "m3u8_native" || f.url && f.url.includes("m3u8"));
      if (format2 && format2.url) {
        return res.json({ url: format2.url, title: data.title, isLive: true });
      }
    }
    const format = data.formats.slice().reverse().find((f) => f.vcodec !== "none" && f.acodec !== "none" && (f.ext === "mp4" || f.ext === "webm"));
    if (format && format.url) {
      return res.json({ url: format.url, title: data.title, isLive: false });
    }
    const anyVideo = data.formats.slice().reverse().find((f) => f.vcodec !== "none");
...
app.get("/api/proxy-video", async (req, res) => {
  ...
  if (useFfmpeg) {
    ...
    (0, import_fluent_ffmpeg.default)().input("testsrc=size=640x360:rate=30").inputFormat("lavfi").outputOptions([
      `-vf`,
      `drawtext=text='${text}':fontcolor=white:fontsize=48:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.5:boxborderw=10`,
      "-f",
      "mp4",
      "-movflags",
  ...
  try {
    ...
    res.setHeader("Access-Control-Allow-Methods", "GET, OPTIONS");
    if (response.body) {
      const readableNodeStream = import_stream.Readable.fromWeb(response.body);
...
function getSimulatedAnalysis(videoName, simulatedOverNumber) {
...
app.post("/api/auth/register-passkey-options", requireAuth, async (req, res) => {
  try {
    const user = req.user;
    if (!user) {
...
app.post("/api/auth/register-passkey-verify", requireAuth, async (req, res) => {
  try {
    const user = req.user;
    if (!user) {
      return res.status(401).json({ error: "Unauthorized" });
    ...
    await db.collection("challenges").doc(user.uid).delete();
    ...
    if (verification.verified && verification.registrationInfo) {
      ...
      const counter = regInfo.counter !== void 0 ? regInfo.counter : regInfo.credential?.counter || 0;
  ...
  try {
    ...
    if (verification.verified && verification.authenticationInfo) {
      ...
      const mockUser = {
        ...
        metadata: {
          creationTime: profile?.created_at || (/* @__PURE__ */ new Date()).toISOString(),
          lastSignInTime: (/* @__PURE__ */ new Date()).toISOString()
        }
      };
      res.json({
        success: true,
...
app.delete("/api/users/delete", requireAuth, async (req, res) => {
  res.json({ success: true, message: "Handled client-side" });
});
var PORT = 3e3;
app.post("/api/notify", requireAuth, async (req, res) => {
  try {
    ...
    if (tokens.length > 0) {
      try {
        response = await (0, import_messaging.getMessaging)().sendEachForMulticast(message);
      } catch (e) {
    ...
    try {
      ...
      profilesSnap.forEach((doc) => {
        ...
        batch.set(notifRef, {
          ...
          is_read: false,
          created_at: import_firestore.FieldValue.serverTimestamp(),
          action_url: matchId ? `/?match=${matchId}` : void 0
        });
      });
      await batch.commit();
    } catch (e) {
...
if (!_global.__NMS_STARTED__) {
  ...
  const nms = new import_node_media_server.default(nmsConfig);
  try {
    nms.run();
...
app.use((0, import_http_proxy_middleware.createProxyMiddleware)({
  ...
  router: (req) => {
    const hostQuery = req.query?.flvHost;
...
async function startServer() {
  const isProduction = process.env.NODE_ENV === "production" || process.env.PM2_HOME !== void 0 || process.argv.some((arg) => arg.includes("server.cjs"));
  if (!isProduction) {
  ...
  wss.on("connection", (ws, req) => {
    ...
    if (url.pathname === "/api/stream/youtube") {
      ...
      const ffmpegPath = import_ffmpeg.default ? import_ffmpeg.default.path : "ffmpeg";
      const ffmpegProcess = (0, import_child_process.spawn)(ffmpegPath, [
        "-f",
        "webm",
        "-i",
        "-",
      ...
      ws.on("message", (msg) => {
        try {
          ...
          wss.clients.forEach((client) => {
            if (client !== ws && client.readyState === 1 && client.matchId === matchId) {
