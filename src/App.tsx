        ...
        <div className="flex-1 lg:flex-[2.8] flex flex-col border-r border-[#2a2a2e] overflow-y-auto custom-scrollbar bg-[#09090b]">
          ...
          <div className="p-6 flex flex-col gap-4">
            ...
            <div className="relative rounded-2xl border border-[#2a2a2e] bg-black overflow-hidden shadow-2xl flex flex-col group/player">
              ...
              <div className="aspect-video relative w-full flex items-center justify-center bg-[#050508]">
                  ...
                  <>
                    ...
                    {ocrEnabled && (
                      <div 
                        ...
                        className="absolute border-2 border-dashed border-emerald-500 bg-emerald-500/10 pointer-events-none transition-all duration-300 shadow-[0_0_20px_rgba(16,185,129,0.3)] z-40"
                        style={{
                          left: `${roiX}%`,
                          top: `${roiY}%`,
                          width: `${roiWidth}%`,
                      ...
                      >
                        ...
                        <div className="absolute -top-5.5 left-0 bg-[#0c0c0f] border border-emerald-500/30 text-[8px] text-emerald-400 px-2 py-0.5 rounded font-mono font-extrabold uppercase tracking-widest select-none shadow-md flex items-center gap-1">
                          <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-ping"></span>
                          <span>Live OCR Area: {roiWidth}% x {roiHeight}%</span>
                        </div>

                        {/* Real-time scan readout text overlay badge inside ROI */}
                        {ocrRecognizedText && (
                          <div className="absolute bottom-1 right-2 font-mono text-[9.5px] font-bold text-white bg-[#0a0a0d] border border-[#2a2a32] px-2 py-0.5 rounded shadow-lg uppercase tracking-wider">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (!selectedMatch) return;
                        const log = `[${new Date().toLocaleTimeString()}] [FAST-FORWARD] Initializing high-speed OCR grid segment batch generation...`;
                        setOcrLogs(prev => [log, ...prev]);

                        let count = 0;
                        const deliveriesToProcess = selectedMatch?.deliveries || [
                          { over: 1, ball: 1, startTime: 2, endTime: 8, bowlerReleaseTime: 4, ballOutcome: "1 Run", runs: 1, wicket: false, extra: false, bowler: "Anrich Nortje", batsman: "Quinton de Kock", description: "Good single." },
                          { over: 1, ball: 2, startTime: 10, endTime: 16, bowlerReleaseTime: 12, ballOutcome: "4 Runs", runs: 4, wicket: false, extra: false, bowler: "Anrich Nortje", batsman: "Quinton de Kock", description: "Flimsy outswinger dispatched for four!" },
                          { over: 1, ball: 3, startTime: 18, endTime: 24, bowlerReleaseTime: 20, ballOutcome: "Wicket", runs: 0, wicket: true, extra: false, bowler: "Anrich Nortje", batsman: "Quinton de Kock", description: "Clean bowled!" }
                        ];

                        deliveriesToProcess.forEach((d, idx) => {
                          setTimeout(() => {
                            extractVideoSegmentDirect(selectedMatch.videoUrl, d.startTime, d.endTime)
                              .then((blob) => {
                                const url = URL.createObjectURL(blob);
                                const newClip = {
                                  id: `ff_ocr_clip_${d.over}_${d.ball}_${Date.now()}`,
                                  name: `Scorecard Over ${d.over}.${d.ball} Clip`,
                                  url,
                                  over: d.over,
                                  ball: d.ball,
                                  bowler: d.bowler,
                                ...
                                const success = `[${new Date().toLocaleTimeString()}] [BATCH SUCCESS] Fully processed Over ${d.over}.${d.ball}. Lossless MP4 clip generated and synchronized!`;
                      ...
                      className="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white transition flex items-center gap-1 cursor-pointer"
                      title="Run quick auto-segment scan over entire stream using target ROI coordinates"
                    >
