  ...
  try {
    ...
    if (permission === 'granted') {
      const token = await getToken(messaging, {
        vapidKey: 'BJwT2Hn-eS34D5eN1aM-Y2G3aNf-t5q0xW_w0dI-Uq8N09H8N3N9g4v-k2f3p6I_n4n5p9x-V2n3w1f5t1H3e3q' // We can just provide a dummy one if it doesn't exist, but it's better to fetch from env. Wait, we don't have vapid key. We can just use getToken without vapidKey, or just rely on standard web push. Let's just use generic FCM.
      });
      return token;
    }
  } catch (e) {
...
if (code.includes(target)) {
  ...
  console.log('Patched requestNotificationPermission.');
} else {
